  ...
  return (
    <div className="animate-entrance">
      <div className="header-section" style={{ marginBottom: '32px' }}>
        <div>
          <h1 className="page-title">Incident Response<span className="text-gradient"></span></h1>
              ...
              {data?.content.map((incident, idx) => (
                <tr key={incident.id} style={{ borderTop: idx > 0 ? '1px solid var(--border-subtle)' : 'none' }}>
                  ...
                  <td style={{ padding: '16px 0' }}>
                    <span className={`badge ${incident.severity === 'P1' || incident.severity === 'CRITICAL' ? 'critical' : 'high'}`}>{incident.severity}</span>
                  </td>
                  ...
                  <td style={{ padding: '16px 0' }}>
                    <span style={{ color: incident.status === 'ACTIVE' || incident.status === 'DETECTED' ? 'var(--status-critical)' : 'var(--status-high)' }}>{incident.status}</span>
                  </td>
                </tr>
              ))}
            </tbody>
